...
PMIX = function (dados, lags, n, seriePlot, facPlot) {
  ...
  criterios = criterios(serieH, lags, dpRes)
  AIC = criterios$AIC
  BIC = criterios$BIC
  ...
  dados = dados[-4]
  data = paste0("PMIX ", dados," (", p, ", ", q, ", ", P, ", ", Q, ") ", data)
  ...
  tabelaDadosA = data.frame(ciclos, somRes, AIC, BIC)
  colnames(tabelaDadosA) = c("Ciclos", "SomRes", "AIC", "BIC")
  rownames(tabelaDadosA) = "Medidas"
  write.csv2(tabelaDadosA, paste0(data, "/Medidas.csv"))
  ...
  final = list(serieH = serieH, serieS = serieS, mediaH = mediaH, mediaS = mediaS, dpH = dpH, dpS = dpS, ciclos = ciclos, somRes = somRes, AIC = AIC, BIC = BIC)
  return (final)
}