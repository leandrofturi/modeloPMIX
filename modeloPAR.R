...
residuos = function(serieHist, serieSint, tablePhi, varEpsilon, p){                 # CALCULO DOS RESIDUOS DA SERIE
  n = length(serieSint)
  serieSint = log(serieSint)
  mediaNormal = apply(log(serieHist), 2, mean)
  sdNormal = apply(log(serieHist), 2, sd)
  serieSint = t((t(serieSint) - mediaNormal) / sdNormal)
  serie = as.vector(t(serieSint))
  ...
  for(t in ((p+1):n)){
    mes = t %% 12
          if(mes == 0) mes = 12
    residuo[t] = sum(serie[(t-1):(t-p)] * tablePhi[, mes])
    residuo[t] = serie[t] - residuo[t]
  }
  tableResiduos = matrix(residuo, ncol = 12, byrow = T)
  tableResiduos = tableResiduos[-1, ]
  
  return(tableResiduos)
}


assimetria = function(serie){
  n = length(serie)/12
  mediaMensal = apply(serie, 2, mean)
  varMensal = apply(serie, 2, var)
  assimetria = numeric(12)
  
  serie = t((t(serie) - mediaMensal)) ^ 3
  assimetria = apply(serie, 2, sum) / n
  assimetria = t(t(assimetria) / (varMensal ^ (3/2)))
  assimetria = as.vector(assimetria)
  
  return(assimetria)
}


modeloPAR = function(serie, p, n, seriePlot){
